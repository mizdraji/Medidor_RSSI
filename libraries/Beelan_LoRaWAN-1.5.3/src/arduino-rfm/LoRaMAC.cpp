 									...
 									sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message_Rx, sSettings *LoRa_Settings)
{
  ...
  //Get data
	...
	*RFM_Command = NO_RFM_COMMAND;
	...
	#elif AU_915
    LoRa_Settings->Channel_Rx=SF12BW500;
    LoRa_Settings->Datarate_Rx=CH8;
  #endif
		LORA_Receive_Data(Data_Rx, Session_Data, OTAA_Data, Message_Rx, LoRa_Settings);
		*RFM_Command = NO_RFM_COMMAND;
	}
...
void LORA_Send_Data(sBuffer *Data_Tx, sLoRa_Session *Session_Data, sSettings *LoRa_Settings)
{
  ...
  #ifdef DEBUG3
      Serial.println("LORA_Send_Data - Message struct");
  #endif
  //Initialise Message struct for a transmit message
  sLoRa_Message Message;
  #ifdef DEBUG3
      Serial.println("LORA_Send_Data - Message struct - header");
  #endif
  ...
  //Unconfirmed
  #ifdef DEBUG3
      Serial.println("LORA_Send_Data - confirmation?");
  #endif
  ...
  #ifdef DEBUG3
      Serial.println("LORA_Send_Data - Build the Radio Package");
  #endif
  ...
  #ifdef DEBUG3
      Serial.println("LORA_Send_Data - Load frame control");
  #endif
  //Load frame control
  ...
  RFM_Package.Counter = 8;
    ...
    #ifdef DEBUG3
      Serial.println("LORA_Send_Data - Data_Tx->Counter > 0x00 ?");
    #endif
  ...
  RFM_Package.Data[8] = LoRa_Settings->Mport;
    ...
    #ifdef DEBUG3
      Serial.println("LORA_Send_Data - Encrypt_Payload");
  #endif
    //Encrypt the data
    Encrypt_Payload(Data_Tx, Session_Data->AppSKey, &Message);

    #ifdef DEBUG3
      Serial.println("LORA_Send_Data - load data");
  #endif
    //Load Data
  ...
  #ifdef DEBUG3
    Serial.println("LORA_Send_Data - Construct_Data_MIC");
  #endif
  //Calculate MIC
  Construct_Data_MIC(&RFM_Package, Session_Data, &Message);

  #ifdef DEBUG3
    Serial.println("LORA_Send_Data - Load MIC in package");
  #endif
  //Load MIC in package
  for(i = 0; i < 4; i++)
  {
  ...
  #ifdef DEBUG3
    Serial.println("LORA_Send_Data - RFM_Send_Package");
  #endif
  //Send Package
  RFM_Send_Package(&RFM_Package, LoRa_Settings);

  #ifdef DEBUG3
    Serial.println("LORA_Send_Data - Raise Frame counter");
  #endif
  //Raise Frame counter
  ...
  #ifdef DEBUG3
    Serial.println("LORA_Send_Data - Change channel for next message if hopping is activated");
  #endif
...
*****************************************************************************************
*/
void LORA_Send_ACK(sBuffer *Data_ACK, sLoRa_Session *Session_Data, sSettings *LoRa_Settings)
{
  //Define variables
  unsigned char i;
  ...
  //confirmed data up = 0x80
  Message.MAC_Header = Message.MAC_Header | 0x40;
  
  #ifdef DEBUG5
      Serial.println("LORA_Send_ACK - Build the Radio Package");
  #endif
  //Build the Radio Package
  //Load mac header
  ...
  #ifdef DEBUG5
      Serial.println("LORA_Send_ACK - Load frame control");
  #endif
  //Load frame control
  Message.Frame_Control = (Message.Frame_Control | 0x20);
  ...
  RFM_Package.Counter = 8;
    ...
    #ifdef DEBUG5
      Serial.println("LORA_Send_ACK - Data_Tx->Counter > 0x00 ?");
    #endif
  //If there is data load the Frame_Port field
  //Encrypt the data and load the data
  if(Data_ACK->Counter > 0x00)
  {
  //Load Frame port field
  RFM_Data[8] = Message.Frame_Port;
  //RFM_Data[8] = LoRa_Settings->Mport;
    ...
    #ifdef DEBUG5
      Serial.println("LORA_Send_ACK - Encrypt_Payload");
  #endif
    //Encrypt the data
    Encrypt_Payload(Data_ACK, Session_Data->AppSKey, &Message);

    #ifdef DEBUG5
      Serial.println("LORA_Send_ACK - load data");
  #endif
    //Load Data
    for(i = 0; i < Data_ACK->Counter; i++)
    {
      RFM_Data[RFM_Package.Counter + i] = Data_ACK->Data[i];
    }

    //Add data Length to package counter
    RFM_Package.Counter = RFM_Package.Counter + Data_ACK->Counter;
  }

  #ifdef DEBUG5
    Serial.println("LORA_Send_ACK - Construct_Data_MIC");
  #endif
  //Calculate MIC
  Construct_Data_MIC(&RFM_Package, Session_Data, &Message);

  #ifdef DEBUG5
    Serial.println("LORA_Send_ACK - Load MIC in package");
  #endif
  //Load MIC in package
  for(i = 0; i < 4; i++)
  {
  ...
  #ifdef DEBUG5
    Serial.println("LORA_Send_ACK - RFM_Send_Package");
  #endif
  //Send Package
  RFM_Send_Package(&RFM_Package, LoRa_Settings);

  #ifdef DEBUG5
    Serial.println("LORA_Send_ACK - Raise Frame counter");
  #endif
  //Raise Frame counter
  if(*Session_Data->Frame_Counter != 0xFFFF)
  {
  ...
  #ifdef DEBUG5
    Serial.println("LORA_Send_ACK - Change channel for next message if hopping is activated");
  #endif
  //Change channel for next message if hopping is activated
  if(LoRa_Settings->Channel_Hopping == 0x01)
  {
...
void LORA_Receive_Data(sBuffer *Data_Rx, sLoRa_Session *Session_Data, sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message, sSettings *LoRa_Settings)
{
  ...
  unsigned char Address_Check;
	...
	#ifdef DEBUG3
        Serial.println("LORA_Receive_Data - Message_Status = NO_MESSAGE");
    #endif
	...
	{
		#ifdef DEBUG3
        	Serial.println("LORA_Receive_Data - RFM_Switch_Mode");
    	#endif
		//Switch RFM to standby
		RFM_Switch_Mode(RFM_MODE_STANDBY);

		#ifdef DEBUG3
        	Serial.println("LORA_Receive_Data - Message_Status = NEW_MESSAGE");
    	#endif
	...
	{
		#ifdef DEBUG3
        	Serial.println("LORA_Receive_Data - Message_Status = RFM_Get_Package");
    	#endif
		...
		{
			#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - RFM_Continuous_Receive");
    		#endif
	...
	{
		#ifdef DEBUG3
        	Serial.println("LORA_Receive_Data - Message_Status == CRC_OK");
    	#endif
		//Get MAC_Header
    	Message->MAC_Header = RFM_Data[0];

		//Data message
		if(Message->MAC_Header == 0x60 || Message->MAC_Header == 0xA0)
		{
			#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - Get MAC_Header from received data");
    		#endif
			...
			#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - Construct_Data_MIC");
    		#endif
			//Calculate MIC
			Construct_Data_MIC(&RFM_Package, Session_Data, Message);
			...
			#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - Compare MIC");
    		#endif
            //Compare MIC
			for(i = 0x00; i < 4; i++)
			{
			...
			#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - Check MIC");
    		#endif
			//Check MIC
      		if(MIC_Check == 0x04)
      		{
      		  #ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - MIC_OK");
    		  #endif
      		...
      		{
      		  #ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - WRONG_MESSAGE");
    		  #endif
      		...
      		#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - Check address");
    		  #endif
      		//Check address
      		if(MIC_Check == 0x04)
      		{
		  	...
		  	{
		  		#ifdef DEBUG3
        			Serial.println("LORA_Receive_Data - ADDRESS_OK");
    			#endif
			...
			if(Message_Status == ADDRESS_OK)
			{
				...
				{
					#ifdef DEBUG3
        				Serial.println("LORA_Receive_Data - Data_Rx->Counter = 0x00");
    				#endif
				...
				{
					#ifdef DEBUG3
        				Serial.println("LORA_Receive_Data - Frame_Port = RFM_Data[8]");
    				#endif
				...
				{
					#ifdef DEBUG3
        				Serial.println("LORA_Receive_Data - Copy and decrypt the data");
    				#endif
          ...
          #ifdef DEBUG3
          	Serial.println("LORA_Receive_Data - MESSAGE_DONE");
      		#endif
  				
          if(Message->MAC_Header == 0xA0)
          {
            Message_Status = MESSAGE_DONE_ACK;
          }else {
		...
		{
			#ifdef DEBUG3
        		Serial.println("LORA_Receive_Data - Message_Status == WRONG_MESSAGE");
    		#endif
			Data_Rx->Counter = 0x00;
 		}
	}
