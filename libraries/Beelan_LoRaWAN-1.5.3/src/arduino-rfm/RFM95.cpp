...
{
  #ifdef DEBUG1
    Serial.print("- (W) RFM_change_SF_BW (SFx CRC On) -");
  #endif
	RFM_Write(RFM_REG_MODEM_CONFIG2, (_SF << 4) | 0b0100); //SFx CRC On
  #ifdef DEBUG1
    Serial.print("- (W) RFM_change_SF_BW (x kHz 4/5 coding rate explicit header mode) -");
  #endif
	RFM_Write(RFM_REG_MODEM_CONFIG1,(_BW << 4) | 0x02); //x kHz 4/5 coding rate explicit header mode

  if(_SF==12 || _SF==11){
    #ifdef DEBUG1
      Serial.print("- (W) RFM_change_SF_BW (Low datarate optimization on AGC auto on) -");
    #endif
    RFM_Write(RFM_REG_MODEM_CONFIG3, 0x0C); //Low datarate optimization on AGC auto on
  }
  else{
    #ifdef DEBUG1
      Serial.print("- (W) RFM_change_SF_BW (Mobile node, low datarate optimization on AGC acorging to register LnaGain) -");
    #endif
...
static void RFM_Change_Channel(unsigned char Channel){
  ...
  if (Channel <= 0x07){
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Change_Channel (Ch<=0x07 / RFM_Write(RFM_REG_FR_MSB + i, pgm_read_byte(&(LoRa_TX_Freq[Channel][i])));) -");
    #endif
    for(unsigned char i = 0 ; i < 3 ; ++i)
    ...
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Change_Channel (Ch>=0x08 & Ch<=0x0F / RFM_Write(RFM_REG_FR_MSB + i, pgm_read_byte(&(LoRa_RX_Freq[Channel - 0x08][i])))) -");
    #endif
    for(unsigned char i = 0 ; i < 3 ; ++i) { 
...
bool RFM_Init()
{
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Switch RFM to sleep) -");
  #endif
  //Switch RFM to sleep
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Set RFM in LoRa mode) -");
  #endif
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Switch RFM to standby) -");
  #endif
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Switch LNA boost on) -");
  #endif
  //Switch LNA boost on
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Rx Timeout set to 37 symbols) -");
  #endif
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Preamble length set to 8 symbols) -");
  #endif
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Set LoRa sync word) -");
  #endif
  //Set LoRa sync word
  RFM_Write(RFM_REG_SYNC_WORD, 0x34);

  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Set FIFO pointers / TX base address) -");
  #endif
  //Set FIFO pointers
  //TX base address
  RFM_Write(0x0E,0x80);
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Init (Set FIFO pointers / RX base address) -");
  #endif
  //Rx base address
  RFM_Write(0x0F,0x00);
  return 1;
...
void RFM_Set_Tx_Power(int level, int outputPin)
{
  if (RFO_PIN == outputPin) {
    ...
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Set_Tx_Power (RFM_Write(RFM_REG_PA_CONFIG, 0x70 | level)) -");
    #endif
    ...
    if (level > 17) {
      ...
      #ifdef DEBUG1
        Serial.print("- (W) RFM_Set_Tx_Power (High Power +20 dBm Operation (Semtech SX1276/77/78/79 5.4.3.)) -");
      #endif
      ...
      #ifdef DEBUG1
        Serial.print("- (W) RFM_Set_Tx_Power (Default value PA_HF/LF or +17dBm) -");
      #endif
    ...
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Set_Tx_Power (PA Boost mask) -");
    #endif
...
void RFM_Set_OCP(uint8_t mA)
{
  ...
  #ifdef DEBUG1
    Serial.print("- RFM_Set_OCP - (RFM_Write(RFM_REG_OCP, 0x20 | (0x1F & ocpTrim)))");
  #endif
...
void RFM_Send_Package(sBuffer *RFM_Tx_Package, sSettings *LoRa_Settings)
{
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Switch DIO0 to TxDone) -");
  #endif
  //Switch DIO0 to TxDone
  RFM_Write(RFM_REG_DIO_MAPPING1, 0x40);

  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Set IQ to normal values) -");
  #endif
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Set payload length to the right length) -");
  #endif
  //Set payload length to the right length
  RFM_Write(RFM_REG_PAYLOAD_LENGTH,RFM_Tx_Package->Counter);

  #ifdef DEBUG2
    Serial.print("- (R) RFM_Send_Package (Get location of Tx part of FiFo) -");
  #endif
  //Get location of Tx part of FiFo
  RFM_Tx_Location = RFM_Read(0x0E);

  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Set SPI pointer to start of Tx part in FiFo) -");
  #endif
  //Set SPI pointer to start of Tx part in FiFo
  RFM_Write(RFM_REG_FIFO_ADDR_PTR, RFM_Tx_Location);

  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Write Payload to FiFo) -");
  #endif
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Switch RFM to Tx) -");
  #endif
  //Switch RFM to Tx
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Send_Package (Clear interrupt) -");
  #endif
  //Clear interrupt
...
message_t RFM_Single_Receive(sSettings *LoRa_Settings)
{
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Single_Receive (Change DIO 0 back to RxDone) -");
  #endif
  //Change DIO 0 back to RxDone
  RFM_Write(RFM_REG_DIO_MAPPING1, 0x00);

  #ifdef DEBUG1
    Serial.print("- (W) RFM_Single_Receive (Invert IQ Back) -");
  #endif
  //Invert IQ Back
  ...
  {
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Single_Receive (Clear interrupt register) -");
    #endif
...
{
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Continuous_Receive (Change DIO 0 back to RxDone and DIO 1 to rx timeout) -");
  #endif
  //Change DIO 0 back to RxDone and DIO 1 to rx timeout
  RFM_Write(RFM_REG_DIO_MAPPING1,0x00);

  #ifdef DEBUG1
    Serial.print("- (W) RFM_Continuous_Receive (Invert IQ Back) -");
  #endif
  //Invert IQ Back
  RFM_Write(RFM_REG_INVERT_IQ, 0x67);
  RFM_Write(RFM_REG_INVERT_IQ2, 0x19);
  ...
  RFM_Change_Channel(CHRX2);
	...
	RFM_Change_Datarate(LoRa_Settings->Datarate_Rx);
	RFM_Change_Channel(LoRa_Settings->Channel_Rx);
  // RFM_Change_Datarate(0x08);
  // RFM_Change_Channel(0x08);
#endif
...
message_t RFM_Get_Package(sBuffer *RFM_Rx_Package)
{
  ...
  unsigned char RFM_Confirmation_ACK = 0x00;

  message_t Message_Status;

  #ifdef DEBUG2
    Serial.print("- (R) RFM_Get_Package (Get interrupt register) -");
  #endif
...
//  if((RFM_Interrupts & 0x20) != 0x20)
  if(((RFM_Interrupts & 0x20) != 0x20) && ((RFM_Interrupts & 0x50) == 0x50))
  {
	  Message_Status = CRC_OK;
    #ifdef DEBUG2
      Serial.print("- (R) RFM_Get_Package (Read start position of received package) -");
    #endif
    RFM_Package_Location = RFM_Read(0x10); /*Read start position of received package*/
    #ifdef DEBUG2
      Serial.print("- (R) RFM_Get_Package (Read length of received package) -");
    #endif
    RFM_Rx_Package->Counter = RFM_Read(0x13); /*Read length of received package*/
      if (RFM_Rx_Package->Counter >= 0x33){ //payload length out of size
        #ifdef DEBUG1
          Serial.print("- (W) RFM_Get_Package WRONG_MESSAGE - (Clear interrupt register) -");
        #endif
        //Clear interrupt register
        RFM_Write(RFM_REG_IRQ_FLAGS,0xE0); 
    ...
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Get_Package (Set SPI pointer to start of package) -");
    #endif

    RFM_Write(RFM_REG_FIFO_ADDR_PTR,RFM_Package_Location); /*Set SPI pointer to start of package*/

    #ifdef DEBUG2
      Serial.print("- (R) RFM_Get_Package / RFM_Rx_Package (data) -");
    #endif
  ...
  {
    #ifdef DEBUG2
      Serial.print("- (R) RFM_Get_Package (WRONG_MESSAGE) -");
    #endif
	  Message_Status = WRONG_MESSAGE;
  }
  ...
  #ifdef DEBUG1
    Serial.print("- (W) RFM_Get_Package (Clear interrupt register) -");
  #endif
  //Clear interrupt register
  RFM_Write(RFM_REG_IRQ_FLAGS,0xE0); 
  //RFM_Write(RFM_REG_IRQ_FLAGS,0xC0); //ACA CAMBIE EL VALOR
...
void RFM_Switch_Mode(unsigned char Mode)
{
    ...
    #ifdef DEBUG1
      Serial.print("- (W) RFM_Switch_Mode (Switch mode on RFM module) -");
    #endif
