...
bool LoRaWANClass::init(void)
{
    ...
    LoRa_Settings.Mote_Class = 0x01; //0x00 is type A, 0x01 is type C
    ...
    Buffer_ACK.Data = Data_ACK;
    Buffer_ACK.Counter = 0x00;
...
void LoRaWANClass::update(void)
{
    ...
    if((RFM_Command_Status == NEW_RFM_COMMAND || RFM_Command_Status == JOIN) && LoRa_Settings.Mote_Class == CLASS_A)
    {
      ...
      if ((Message_Rx.Frame_Control & 0x20) > 0){
        Ack_Status = NEW_ACK;
        #ifdef DEBUG4
            Serial.println("Ack_Status = NEW_ACK");
        #endif   
      }
    ...
    {
        /*#ifdef DEBUG3
            Serial.println("update - CLASS_C");
        #endif*/
       //Transmit
      if(RFM_Command_Status == NEW_RFM_COMMAND)
      {
        #ifdef DEBUG3
            Serial.println("update - LORA_Send_Data");
        #endif     
        //Lora send data
      ...
      { 
        #ifdef DEBUG3
            Serial.println("update - LORA_Receive_Data");
        #endif
        //Get data
      ...
      /*#ifdef DEBUG3
        Serial.println("update - RFM_Command_Status = NO_RFM_COMMAND");
      #endif*/
      RFM_Command_Status = NO_RFM_COMMAND;
    }
