...
bool RFM_Init()
{
  ...
  //Set FIFO pointers
  //TX base address
  RFM_Write(RFM_REG_FIFO_TX_BA_ADDR,0x80);
  //Rx base address
  RFM_Write(RFM_REG_FIFO_RX_BA_ADDR,0x00);
  return 1;
}
...
void RFM_Send_Package(sBuffer *RFM_Tx_Package, sSettings *LoRa_Settings)
{
  ...
  RFM_Tx_Location = RFM_Read(RFM_REG_FIFO_TX_BA_ADDR);
  ...
  //Switch RFM to Tx
  RFM_Write(RFM_REG_OP_MODE,0x83);    //10000011        Transmit (TX)

  //Wait for TxDone
  while(digitalRead(RFM_pins.DIO0) == LOW);

  //Clear interrupt
  RFM_Write(RFM_REG_IRQ_FLAGS,0x08);      //00001000      Clear TxDone interrupt
...
{
  //Change DIO0 back to RxDone and DIO1 to rx timeout
  RFM_Write(RFM_REG_DIO_MAPPING1,0x00);

  //Invert IQ Back
  RFM_Write(RFM_REG_INVERT_IQ, 0x67);     //01100111      bit6=1:I and Q signals are inverted
  RFM_Write(RFM_REG_INVERT_IQ2, 0x19);    //00011001      Set to 0x19 for inverted IQ
...
message_t RFM_Get_Package(sBuffer *RFM_Rx_Package)
{
  ...
  //bit7: RxTimeout
  //bit6: RxDone
  //bit5: Payload CRC Error
  //bit4: Valid Header
  //bit3: TxDone
  //bit2: Cad Detected
  //bit1: Fhss Change Channel
  //bit0: Cad Done  

 
  if((RFM_Interrupts & 0x40)){ //IRQ_RX_DONE_MASK 01000000
    if((RFM_Interrupts & 0x20) != 0x20)  //Check CRC ERROR MASK 00100000
    {
  ...
  RFM_Package_Location = RFM_Read(RFM_REG_FIFO_RX_CU_ADDR);   //Read start position of received package
  RFM_Rx_Package->Counter = RFM_Read(RFM_REG_RX_NB_BYTES);    //Read length of received package
